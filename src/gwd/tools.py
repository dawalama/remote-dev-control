"""Standalone tool definitions and executor for gwd.

Self-contained — zero RDC imports. Tools are defined in OpenAI function
calling format and executed against the project filesystem with sandboxing.
"""
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "path": {"type": "string", "description": "Directory path relative to project root. Use '.' for project root."},
                    "recursive": {"type": "boolean", "description": "If true, list recursively (max 200 entries). Default false."},
                },
                "required": ["path"],
            },
        },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "glob": {"type": "string", "description": "File glob pattern to filter (e.g. '*.py', '*.ts'). Optional."},
                },
                "required": ["pattern"],
            },
        },
...
async def _read_file(
    ...
    numbered = [f"{i:>5}\t{line}" for i, line in enumerate(selected, start=start + 1)]
    return "\n".join(numbered), False
...
async def _search_files(
    ...
    if glob:
        cmd = ["grep", "-rn", "--include", glob, "-E", pattern, str(target)]
    ...
    try:
        ...
        if len(lines) > 100:
            lines = lines[:100]
            lines.append(f"... (showing first 100 of {len(output.strip().split(chr(10)))} matches)")
