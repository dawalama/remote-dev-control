...
# ── Ensure rdc is in PATH for setup ──────────────────────────────────────────
RDC_BIN=""
if command_exists rdc; then
  RDC_BIN="rdc"
elif [ -f "$RDC_DIR/.venv/bin/rdc" ]; then
  RDC_BIN="$RDC_DIR/.venv/bin/rdc"
  export PATH="$RDC_DIR/.venv/bin:$PATH"
fi
...
if [ -n "$RDC_BIN" ]; then
  ...
  warn "Could not find rdc CLI. Skipping guided setup."
  warn "Run 'rdc setup' manually after adding .venv/bin to your PATH."
fi
...
echo -e "  ${BOLD}Start:${NC}  rdc server start"
echo -e "  ${BOLD}Open:${NC}   http://localhost:8420"
echo -e "  ${BOLD}Add:${NC}    rdc add ~/my-project"
echo -e "  ${BOLD}Rerun:${NC}  rdc setup"
echo ""
